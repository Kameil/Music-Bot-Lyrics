...
class lyrics(commands.Cog): 
    ...
    async def stop(self, inter: discord.Interaction):
        ...
        # check: admin, mod (manage msgs) or in vc
        member: discord.Member = inter.user # type: ignore
        is_admin = member.guild_permissions.administrator
        is_mod = member.guild_permissions.manage_messages
        in_vc = member.voice and member.voice.channel
        
        if not (is_admin or is_mod or in_vc):
            await inter.response.send_message(
                "You need to be an admin, moderator or in a voice channel to use this command.",
                ephemeral=True,
            )
            return
